...
const carritoLs = () =>{
    let llamado = JSON.parse(localStorage.getItem("productos-en-carrito"))
    if(typeof llamado !== "undefined" && !isNaN(llamado) ){
        return(llamado)
    }else{
        llamado = []
        localStorage.setItem("productos-en-carrito", JSON.stringify(llamado))
        return (llamado)
